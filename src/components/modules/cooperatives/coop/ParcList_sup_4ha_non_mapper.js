...
const url = BaseUrl();
function ParcListSup4haNonMapper(){
    ...
    const fetchData = () => {
        axios.get(url+'/ParcelleList_sup_4ha_projet/')
          .then((resp) => {
    ...
    const onSearchParcelle=(event)=>{
        try {
          axios.get(url+'/ParcelleList_sup_4ha_projet/?q='+event.target.value+'&co='+coopID).then((resp)=>{
   ...
   const onSubmitExporteData=(e)=>{
     ...
     try {
       axios.get(url+'/export-parcelle-cooperative-non-mapper/?format='+format_exp+'&campagne='+campagne_exp+'&coopID='+coopID,
         {responseType:'blob'}
       ).then((response)=>{
           if (format_exp == 'PDF'){
             fileDownload(response.data,'liste_parcelles_sup_4ha_non_mapper.pdf');
           }else{
             fileDownload(response.data,'liste_parcelles_sup_4ha_non_mapper.xlsx')
           }
...
//console.log(parcelleList);
    ...
    return(
        <Fragment>
            ...
            <h2 className="text-bold text-1100 mb-5">Liste des Parcelles Supérieur à 4ha Non Mapper ({totalParc})</h2>
                            ...
                            <div className="card">
                                ...
                                <div className="card-body p-2">
                                    <h3 className="card-title text-center text-warning"><Link
                                        to={`/coops/parcelles-list-sup-4ha/${coopID}/`}>Liste</Link></h3>
                                </div>
                            </div>
                        </div>
                        <div className="col-md-6">
...
export default ParcListSup4haNonMapper;